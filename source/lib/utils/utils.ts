...
export function getNodeTierName(
  nodeType: DruidNodeType,
  serviceTier?: string,
): string {
...
export function getInstanceTypeInfo(instanceType: string): Ec2InstanceTypeInfo {
  const instanceTypeInfo = JSON.parse(
    fs
      .readFileSync(`lib/instance-types/${instanceType}.json`, "utf-8")
      .toString(),
  );
  ...
  return {
    ...
    arch: instanceTypeInfo.InstanceTypes[0].ProcessorInfo
...
export function ifUndefined<T, R>(
  value: T | undefined,
  defaultValue: R,
): T | R {
...
export function mapProcessTypeToNodeType(
