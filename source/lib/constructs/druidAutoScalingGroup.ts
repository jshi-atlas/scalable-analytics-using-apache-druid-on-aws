...
export class DruidAutoScalingGroup extends Construct {
  ...
  public constructor(
    ...
    const nodeTierName = utils.getNodeTierName(
      props.nodeType,
      props.serviceTier,
    );
  ...
  private createUserData(
    ...
    const templateVariables: Record<string, string> = {
      ...
      DRUID_EXTENSIONS: JSON.stringify(
        asgContext.clusterParams.druidExtensions,
      ),
      DRUID_COMPONENT: nodeTierName,
      REGION: cdk.Aws.REGION,
      STACK_NAME: cdk.Aws.STACK_NAME,
      RESOURCE_NAME: (asg.node.defaultChild as as.CfnAutoScalingGroup)
        .logicalId,
      ...
      GRACEFUL_TERMINATION_PARAM_NAME:
        this.gracefulTerminationParam.parameterName,
      ADMIN_USER_SECRET_NAME:
        asgContext.rdsMetadataConstruct.druidAdminUserSecret.secretName,
      SYSTEM_USER_SECRET_NAME:
        asgContext.rdsMetadataConstruct.druidInternalSystemUserSecret
          .secretName,
      ...
      COMMON_RUNTIME_CONFIG: JSON.stringify(
        utils.ifUndefined(
          asgContext.clusterParams.druidCommonRuntimeConfig,
          "",
        ),
      ),
