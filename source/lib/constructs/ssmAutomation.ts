...
export class SSMAutomation extends Construct {
  ...
  private createSSMAutomationRole(
    ...
    const role = new iam.Role(
      this,
      `${serviceName}HostTerminationAutomationRole`,
      {
  ...
  private createSSMAutomationDocument(
    ...
    return new ssm.CfnDocument(
      this,
      `${serviceName}${HOST_TERMINATION_DOC_SUFFIX}`,
      {
  ...
  private createEventBridgeRule(
    ...
    return new events.CfnRule(this, `${serviceName}-EC2TerminationEventRule`, {
      description:
        "Event to trigger SSM to run document when hosts are terminated",
      state: "ENABLED",
      eventPattern: {
