...
export abstract class DruidEksBase extends Construct {
  ...
  protected createDruidSecrets(metadataDb: MetadataStore): void {
    ...
    const externalSecretsChart = this.eksCluster.addHelmChart(
      `${addOnName}-chart`,
      {
            ...
            ? [
                {
                  ...
                  remoteRef: {
                    key: this.props.baseInfra.oidcIdpClientSecret.secretName,
  ...
  private createDruidCluster(metadataDb: MetadataStore): void {
    const druidImageAsset = new ecrassets.DockerImageAsset(
      this,
      "druid-aws-image",
      {
    ...
    const commonTemplateVariables: Record<string, any> = {
      ...
      oidc_discovery_uri:
        this.props.druidClusterParams.oidcIdpConfig?.discoveryURI,
