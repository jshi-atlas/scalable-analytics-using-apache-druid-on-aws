...
export class DruidEks extends DruidEksBase {
  ...
  public override deployDruid(
    ...
    const cfnAddon = new eks.CfnAddon(
      this.eksCluster.stack,
      `${addonName}-addon`,
      {
        addonName,
    ...
    if (nodeGroupConfig.query.minNodes > 0) {
      queryNodeGroup = this.eksCluster.addNodegroupCapacity(
        "query-node-group",
        {
    ...
    if (nodeGroupConfig.master.minNodes > 0) {
      masterNodeGroup = this.eksCluster.addNodegroupCapacity(
        "master-node-group",
        {
    ...
    const druidClusterManifest = mustache.render(
      ...
      ),
      {
        ...
        coordinator_processor_count: Math.ceil(
          utils.getInstanceTypeInfo(nodeGroupConfig.master.instanceType).cpu /
            2,
        ),
  ...
  private getCommonDataNodeProps(
    ...
    return {
      ...
      processor_count: Math.ceil(
        utils.getInstanceTypeInfo(nodeGroupConfig[nodeType].instanceType).cpu /
          2,
      ),
  ...
  private createDataNodeGroup(
    ...
    if (nodeGroupConfig[nodeType].minNodes > 0) {
      ...
      const minSizePerAz =
        nodeGroupConfig[nodeType].minNodes / availabilityZoneCnt;
