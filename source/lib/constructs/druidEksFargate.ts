...
export class DruidEksFargate extends DruidEksBase {
  ...
  public override createEksCluster(): eks.ICluster {
    ...
    const loggingNamespaceResource = cluster.addManifest(
      `${loggingNamespace}-ns`,
      {
        kind: "Namespace",
        apiVersion: "v1",
  ...
  public override deployDruid(
    ...
    const [middleManagerTiers, historicalTiers] =
      this.createDataTiers(fargateConfig);
  ...
  private createDataTiers(
    fargateConfig: EksFargateConfig,
  ): [unknown[], unknown[]] {
    ...
    Object.keys(fargateConfig).forEach((processType) => {
      ...
      const matchResult = processType.match(
        /^(historical|middleManager)(\w*)$/,
      );
