...
export class DruidEc2Stack extends DruidStack {
    ...
    addCfnGuardSuppression(securityGroup, [
      {
        ...
        reason:
          "Allowing to talk to Druid peers and download necessary dependencies from internet",
      },
    ]);
    ...
    addCfnNagSuppression(appLoadBalancer, [
      {
        id: "W52",
        reason:
          "Elastic Load Balancer V2 access logging is not supported on environment agnostic stack",
      },
    ]);
  ...
  private createCustomLifecycleHookParams(
    autoScalingPolicy?: AutoScalingPolicy,
  ): CustomLifecycleHookParams | undefined {
    return autoScalingPolicy?.customLifecycleHookParams
      ? {
          defaultResult:
            autoScalingPolicy.customLifecycleHookParams.defaultResult ??
            autoscaling.DefaultResult.CONTINUE,
          heartbeatTimeout:
            autoScalingPolicy.customLifecycleHookParams.heartbeatTimeout ??
            INSTANCE_TERMINATION_TIMEOUT,
        }
      : undefined;
  }
  ...
  private createMasterASG(
    ...
    const customLifecycleHookParams =
      this.createCustomLifecycleHookParams(autoScalingPolicy);

    const masterAsg = this.createAutoScalingGroup(
      asgContext,
      DruidNodeType.MASTER,
      undefined,
      undefined,
      customLifecycleHookParams,
    );
  ...
  private createQueryASG(
    ...
    const queryTierName = utils.getNodeTierName(
      DruidNodeType.QUERY,
      serviceTier,
    );
    const customLifecycleHookParams =
  ...
  private createDataASG(
    ...
    if (autoScalingPolicy?.cpuUtilisationPercent) {
      dataAsg.autoScalingGroup.scaleOnCpuUtilization(
        `${dataTierName}-scale-cpu`,
        {
          targetUtilizationPercent: autoScalingPolicy.cpuUtilisationPercent,
        },
      );
    ...
    if (autoScalingPolicy?.pendingTaskCountScaleSteps) {
      ...
      dataAsg.autoScalingGroup.scaleOnMetric(
        `${dataTierName}-scale-pending-task`,
        {
  ...
  private createHistoricalASG(
    ...
    if (autoScalingPolicy?.cpuUtilisationPercent) {
      historicalAsg.autoScalingGroup.scaleOnCpuUtilization(
        "historical-scale-cpu",
        {
          targetUtilizationPercent: autoScalingPolicy.cpuUtilisationPercent,
        },
      );
      ...
      [
        {
          id: "W12",
          reason:
            "Resource * is required for autoscaling and logs related permissions",
        },
        {
          id: "W76",
          reason:
            "The IAM permissions are required to perform necessary functionalities.",
        },
      ],
      "DefaultPolicy",
    );
  ...
  private importCustomSecret(
    ...
    return secretId.startsWith("arn:")
      ? Secret.fromSecretCompleteArn(this, id, secretId)
      : Secret.fromSecretNameV2(this, id, secretId);
  }
  ...
  private createSecurityGroup(
    ...
    const secGrp = new ec2.SecurityGroup(
      this,
      `${DRUID_SECURITY_GROUP_NAME}-Id`,
      {
        vpc,
  ...
  private attachListenersToLoadBalancers(
    ...
    if (this.certificate) {
      ...
      loadbalancer.addListener(`listener-https-id-v3`, {
        port: 443,
        ...
        "listener-http-id/Resource": [
          {
            id: "W56",
            reason:
              "HTTP listener is required to support optional Route 53 domain",
          },
        ],
  ...
  private createDataTiers(
    ...
    Object.keys(druidConfig).forEach((nodeType) => {
      const matchResult = nodeType.match(
        /^(data|middleManager|historical)(\w*)$/,
      );
      if (matchResult) {
  ...
  private createMonitoringDashboard(
    ...
    this.autoScalingGroupNames.forEach((autoScalingGroupName, nodeTierName) => {
      computeWidgets.push(
        ...this.createAutoScalingGroupWidgets(
          nodeTierName,
          autoScalingGroupName,
        ),
      );
  ...
  private createAlarms(
    ...
    this.autoScalingGroupNames.forEach((autoScalingGroupName, nodeTierName) => {
      computeAlarms.push(
        ...this.createAutoScalingGroupAlarms(
          nodeTierName,
          autoScalingGroupName,
        ),
      );
